...
/* ========= 물체 변수 ========= */
ID3DXMesh* g_Box = nullptr;								// 테스트용 메시
...
bool Setup()
{
	D3DXCreateBox(
		g_Device,
		500.0f / (FLOAT)WIN_WIDTH,
		500.0f / (FLOAT)WIN_HEIGHT,
		((FLOAT)WIN_WIDTH / (FLOAT)WIN_HEIGHT) / 10.0f,
		&g_Box, 
		nullptr);

	// 카메라의 위치와 방향을 조정한다.
	XMVECTOR3 position(0.0f, 0.0f, -2.0f);
